...
L1AXOTreeProducer::L1AXOTreeProducer(edm::ParameterSet const &config)
  ...
  tree_->Branch("axo_score", &anomaly_score, "f");
...
void L1AXOTreeProducer::analyze(edm::Event const &event, edm::EventSetup const &setup) {
  ...
  //save axo score
  edm::Handle<AXOL1TLScoreBxCollection> nnex;
  event.getByToken(scoreToken_, nnex);

  if (nnex.isValid()) {
    *anomaly_score = nnex->at(0, 0).getAXOScore();
    std::cout << "ADDED AXO SCORE TO ROOTFILE: "<< anomaly_score <<std::endl;
